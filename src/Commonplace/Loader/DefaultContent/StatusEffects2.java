...
//    };
    ...
    private final static Floatp zero = () -> 0f;
    private final static Floatp wave10t50l1 = () -> {
    ...
    private final static Floatp wave40l09 = () -> Math.min(0.3f, Vars.state.wave / 100f);
    private final static Floatp wave10t50l05 = () -> {
        int wave = Vars.state.wave - 1;
        if (wave < 10) {
            return wave / 80f;
        } else if (wave < 50) {
    ...
    public static StatusEffect superStop, boostSpeed, hardHit, onePercent, torn, tardy, swift, tension, abyss, gasify, sublimation,
            grow, seethe, strong, back, frenzy, deploy, impatience, loose, shocked, aging, erosion, disturb, fearless, sluggish,
            electricalDisturbance, oscillatoryDisturbance, impact, electricalErosion;
    ...
    public static void load() {
        ...
        superStop = new StatusEffect("super-stop") {{
            show = false;
        ...
        strong = new StatusEffect("strong") {{
        ...
        electricalErosion = new ConditionsStatusEffect("electrical-erosion") {{
            randomSpeed = true;
            randomSpeedMax = 4;
            randomReloadMin = 0.25f;

            damageResult = u -> Math.abs(u.vel.len() - 6) * 2f * (u.isShooting ? 5 : 1) * (u.healTime > 0 ? 10 : 1);
        }};
    ...
    private static void load_peculiarity() {
        peculiarity_health8(1.02f, well);
        peculiarity_health8(1.03f, well);
        peculiarity_health5(1.05f, well);
        peculiarity_health5(1.06f, well);
        peculiarity_health3(1.08f, well);
        peculiarity_health3(1.09f, well);
        peculiarity_health(1.15f, well);
        peculiarity_damage8(1.02f, well);
        peculiarity_damage8(1.03f, well);
        peculiarity_damage5(1.05f, well);
        peculiarity_damage5(1.06f, well);
        peculiarity_damage3(1.08f, well);
        peculiarity_damage3(1.09f, well);
        peculiarity_damage(1.15f, well);
        peculiarity_reload8(1.02f, well);
        peculiarity_reload8(1.03f, well);
        peculiarity_reload5(1.05f, well);
        peculiarity_reload5(1.06f, well);
        peculiarity_reload3(1.08f, well);
        peculiarity_reload3(1.09f, well);
        peculiarity_reload(1.15f, well);
        peculiarity5(1.03f, 1f, 1.03f, 1.02f, well);
        peculiarity5(1.04f, 1f, 1.04f, 1.02f, well);
        peculiarity3(1.07f, 1.03f, 1.06f, 1.02f, well);

        peculiarity8(1.03f, 1, 0.98f, 1, mid);
        peculiarity8(0.99f, 1, 1, 1.02f, mid);
        peculiarity8(1, 0.99f, 1, 1.02f, mid);
        peculiarity8(1, 1, 0.99f, 1.02f, mid);
        peculiarity8(1, 1.03f, 1, 0.98f, mid);
        peculiarity8(1.05f, 1, 0.98f, 0.98f, mid);
        peculiarity8(1.03f, 0.98f, 1, 0.98f, mid);
        peculiarity8(1f, 1 / 1.0225f, 1 / 1.025f, 1.05f, mid);
        peculiarity3(1.2f, 0.84f, 1, 1f, mid);
        peculiarity3(0.84f, 1, 1.2f, 1, mid);
        peculiarity3(1, 1.2f, 1, 0.84f, mid);
        peculiarity3(1, 1.2f, 0.84f, 1, mid);
        peculiarity(1, 1.3f, 0.77f, 1, mid);
        peculiarity(0.9f, 1f, 1.3f, 1, mid);
        peculiarity(1, 1.3f, 1, 1, mid);

        peculiarity_health8(0.98f, bad);
        peculiarity_health3(0.943f, bad);
        peculiarity_health(0.9f, bad);
        peculiarity_damage8(0.98f, bad);
        peculiarity_damage3(0.943f, bad);
        peculiarity_damage(0.9f, bad);
        peculiarity_reload8(0.98f, bad);
    ...
    private static void load_override() {
    ...
    private static void trans(StatusEffect base, StatusEffect face, StatusEffect.TransitionHandler handler) {
        try {
    ...
    private static void peculiarity(float d, float h, float r, float s, Seq<StatusPack> packs) {
    ...
    private static void peculiarity3(float d, float h, float r, float s, Seq<StatusPack> packs) {
    ...
    private static void peculiarity5(float d, float h, float r, float s, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> {
            e.damageMultiplier *= d;
            e.healthMultiplier *= h;
            e.reloadMultiplier *= r;
    ...
    private static void peculiarity8(float d, float h, float r, float s, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> {
            e.damageMultiplier *= d;
            e.healthMultiplier *= h;
            e.reloadMultiplier *= r;
        ...
        packs.addAll(p, p, p, p, p, p, p, p);
    }

    private static void peculiarity_damage(float mul, Seq<StatusPack> packs) {
        packs.add(new StatusPack(e -> e.damageMultiplier *= mul));
    }

    private static void peculiarity_damage3(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.damageMultiplier *= mul);
        packs.add(p, p, p);
    }

    private static void peculiarity_damage5(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.damageMultiplier *= mul);
        packs.addAll(p, p, p, p, p);
    }

    private static void peculiarity_damage8(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.damageMultiplier *= mul);
        packs.addAll(p, p, p, p, p, p, p, p);
    }

    private static void peculiarity_health(float mul, Seq<StatusPack> packs) {
        packs.add(new StatusPack(e -> e.healthMultiplier *= mul));
    }

    private static void peculiarity_health3(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.healthMultiplier *= mul);
        packs.add(p, p, p);
    }

    private static void peculiarity_health5(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.healthMultiplier *= mul);
        packs.addAll(p, p, p, p, p);
    }

    private static void peculiarity_health8(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.healthMultiplier *= mul);
        packs.addAll(p, p, p, p, p, p, p, p);
    }

    private static void peculiarity_reload(float mul, Seq<StatusPack> packs) {
        packs.add(new StatusPack(e -> e.reloadMultiplier *= mul));
    }

    private static void peculiarity_reload3(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.reloadMultiplier *= mul);
        packs.add(p, p, p);
    }

    private static void peculiarity_reload5(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.reloadMultiplier *= mul);
        packs.addAll(p, p, p, p, p);
    }

    private static void peculiarity_reload8(float mul, Seq<StatusPack> packs) {
        StatusPack p = new StatusPack(e -> e.reloadMultiplier *= mul);
        packs.addAll(p, p, p, p, p, p, p, p);
    }

    private static void super_damage(String name, float damageMul, float speedMul, float heal, Floatp damageAdd, Floatp speedAdd, Cons<StatusEffect> change) {
        super_peculiarity(name, damageMul, speedMul, 1, 1, heal, damageAdd, speedAdd, zero, zero, change);
    }

    private static void super_heal(String name, float healMul, float speedMul, float heal, Floatp healAdd, Floatp speedAdd, Cons<StatusEffect> change) {
        super_peculiarity(name, 1, speedMul, healMul, 1, heal, zero, speedAdd, healAdd, zero, change);
    }

    private static void super_reload(String name, float reloadMul, float speedMul, float heal, Floatp reloadAdd, Floatp speedAdd, Cons<StatusEffect> change) {
        super_peculiarity(name, 1, speedMul, 1, reloadMul, heal, zero, speedAdd, zero, reloadAdd, change);
    }

    private static void super_peculiarity(String name, float damageMul, float speedMul, float healMul, float reloadMul, float heal, Floatp damageAdd, Floatp speedAdd, Floatp healAdd, Floatp reloadAdd, Cons<StatusEffect> change) {
        StatusEffect s = new SupperStatus(name) {{
            show = false;
            damage = -heal;
            permanent = true;
            baseDamage = damageMul;
            baseReload = reloadMul;
            baseHealth = healMul;
            baseSpeed = speedMul;
            damageAdder = damageAdd;
            reloadAdder = reloadAdd;
            healthAdder = healAdd;
            speedAdder = speedAdd;
        }};
